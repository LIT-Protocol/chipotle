...
impl BundleCache {
    ...
    fn unpack(&self, bytes: &[u8], cid: &str) -> Result<()> {
        ...
        // Preserved modes come from untrusted input: keep rwx bits (binary
        // entrypoints need exec) but strip setuid/setgid/sticky.
...
mod tests {
    ...
    fn symlink_entries_are_rejected() {
        ...
        {
            let mut builder = tar::Builder::new(&mut tar_bytes);
            let manifest = r#"{"entrypoint": "run.sh"}"#;
            let mut header = tar::Header::new_gnu();
            header.set_size(manifest.len() as u64);
            header.set_mode(0o644);
            header.set_cksum();
            builder
                .append_data(&mut header, "lit.json", manifest.as_bytes())
                .unwrap();
            ...
            link.set_cksum();
            builder
                .append_link(&mut link, "run.sh", "/etc/passwd")
                .unwrap();
            builder.finish().unwrap();
        }
    ...
    fn setuid_bits_are_stripped_but_exec_kept() {
        ...
        let mut tar_bytes = Vec::new();
        {
            ...
            for (name, contents, mode) in [
                ("lit.json", r#"{"entrypoint": ["./run"]}"#, 0o644),
                ("run", "#!/bin/sh\n", 0o4755), // setuid + exec
            ] {
                let mut header = tar::Header::new_gnu();
                header.set_size(contents.len() as u64);
