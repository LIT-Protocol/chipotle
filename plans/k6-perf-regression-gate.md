...
- **Committed baseline:** `k6/baselines/soak.test.json` (a saved summary). The deploy gate passes `baseline_file: ../baselines/soak.test.json` (path is relative to `k6/loadtest/` — where the spec's `open()` resolves).
...
- **Manual baseline refresh:** `k6/update-soak-baseline.sh` runs a measurement soak against staging (create_accounts, no baseline → doesn't gate itself) and writes `k6/baselines/soak.test.json`. Run it + commit via PR when perf legitimately changes (e.g. got substantially faster). Per the decision: baseline updates are a deliberate manual op, not automated.
