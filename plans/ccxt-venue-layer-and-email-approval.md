...
- **Chipotle owns the primitive** (this plan): sending, nonce issuance, attestation signing, and — non-negotiably — **in-TEE verification**. If verification lived in Flows, a Flows compromise could forge approvals and move funds, collapsing the custody story. *(Precise trust boundary, post-review: in-TEE verification stops a compromise of **Flows or the transport** from forging an approval; it does **not** stop a compromise of the approval **service itself**, which holds the signing key — moving signing into an isolated signer/HSM is a tracked follow-up. The code says so plainly rather than overclaiming "cannot forge.")*
...
- `requestEmailApproval({to, summary, assurance, ttlSec, requestHash})` → `{approvalId, otp?}` — issues a single-use nonce, emails a signed approval link hosted by lit-api-server. **`requestHash` binds the exact operation** (amount, destination, venue, …) into the attestation so a "move 0.1 BTC to X" approval can't be replayed to authorize a different movement.
- `checkEmailApproval({approvalId, requestHash, consume})` → `{approved, attestation}` — the attestation is signed by a network attestation key and verified **in-TEE**; the runtime checks the signature, `approvalId`, status, expiry, **and that the attestation's operation hash equals the caller's `requestHash`** (an unbound approval is refused where one is expected — an L1 confirm can't gate a fund move). `consume` (default true) makes an approved record **single-use**: the first approved check spends it, so even the same operation can't be replayed within the TTL. **OTP step-up is brute-force-bounded** (≤5 attempts, then the record locks).
...
### D6.1 Shared approval store — untrusted Neon, signatures for integrity (post-v1)

**Problem (the reason v1's in-memory store is not GA-ready).** The TEE/runtime fleet is horizontally scalable and **redeployed by ping-pong cutover** (new box per deploy — see the deploy-staging ping-pong work). `ApprovalService` keeps pending approvals in `Mutex<HashMap>` (process memory). Two failure modes follow: (a) an approval issued on instance A can't be approved/checked on instance B (no sticky routing), and (b) a cutover lands on a fresh box with an empty store, dropping every pending-but-not-yet-attested approval. The signed attestation is already stateless and survives both; only the *pending* lifecycle is fragile.

**Fix: make the store an untrusted, shared cache; integrity stays in the signature.** Assume the store adversary (a malicious Neon admin, or anyone holding the connection string) can **read and write every row**. Correctness must not depend on that being false.

- **Integrity = the existing `email-approval-v1` signature, not DB access control and not encryption.** The runtime already verifies the secp256k1 attestation over `(approvalId, requestHash, status, expiry)` in-TEE against the pinned pubkey (`verify_approval_attestation`). A store adversary doesn't hold the signing key, so **they cannot forge a new approval** regardless of what they write. This property is storage-agnostic — it holds in RAM, Neon, or anywhere. Moving the store changes nothing about it.
- **Encryption is for OTP confidentiality, NOT the gate.** A "store the encrypted OTP, decrypt-and-compare in the action" scheme is *not* tamperproof on its own: `Lit.Actions.Encrypt` ciphertexts under one access policy are **substitutable** between rows, so an adversary who knows the plaintext of *some* ciphertext (e.g. from an approval they created themselves) can overwrite a victim row with it and pass the compare — forgery. Encryption ≠ authentication. Because a 6-digit OTP is low-entropy, a plain hash in the DB is brute-forced offline in milliseconds; therefore store the OTP as an **HMAC keyed by the TEE-held attestation key** (or `Lit.Actions.Encrypt` it) purely so the admin can't read/self-approve. The approval *decision* remains the signature.
- **Single-use against an untrusted store → push to the execution layer.** An adversary who controls the DB can roll back a `consumed` flag, so no-replay can't be enforced by the store alone. Bind `requestHash` to the **execution-layer idempotency key** (chain nonce, venue `clientOrderId`, withdrawal id); the chain/venue refuses the same operation twice. Approval-layer single-use becomes best-effort defense-in-depth; true anti-replay lives where trusted monotonic state already exists.

**Feasibility.** Actions can't open raw TCP, but **Neon's serverless driver / SQL-over-HTTP** is reachable over `fetch` (via `proxiedFetch` if egress requires it). The connection string is a full-access, non-capability-scoped credential — **seal it** (`venue-credentials-v1`-style) or hold it server-side; never pass it as a plaintext param (logs/exposure), and treat the DB as fully compromised regardless. This supersedes the "shared store is a follow-up" note in Risks.
...
| **M1 — Tier-1 conformance** 🟡 Coinbase authed done | Binance + Coinbase **public** live conformance DONE; **Coinbase authenticated read-only conformance PASSED LIVE 2026-06-11** (ES256-JWT against a real account; literal-`\n` CDP PEMs handled); markets-cache injection DONE. **Remaining:** Binance testnet keys; Exchange-sandbox lifecycle; tiny live Coinbase order behind the manual flag | `k6/correctness/venues-binance.spec.ts`, `venues-coinbase.spec.ts` (built; gated tiers skip without keys); `lit-venues/scripts/verify-live.mjs` **11/11 PASS** |
...
| **M2.5 — Hyperliquid (PKP-native)** 🟡 live-proven | Connector BUILT; signing **pinned to official-SDK vectors**; egress measured (direct US egress fine). **Authenticated testnet lifecycle PASSED LIVE 2026-06-11 with a real agent key**: deep GTC rest→list→cancel AND an IOC fill round-trip (buy 0.01 ETH, position visible, reduce-only close to flat) — twice. Live testing surfaced and fixed the agent-key footgun: reads now auto-resolve the agent's MASTER via `userRole` (agents' own accounts are always empty). **Remaining:** the same suite as Lit Actions on dev (k6 gate, post-deploy); tiny live mainnet order behind manual flag | `k6/correctness/venues-hyperliquid.spec.ts` (built); `verify-live.mjs` Hyperliquid 7/7 incl. fill round-trip |
| **M3 — Email approval** 🟡 built, adversarially hardened | Ops + SDK wrappers + **in-TEE verification (fail-closed)** BUILT, now binding **(a) the attestation signature, (b) the approval id, (c) status/expiry, AND (d) a caller-supplied operation `requestHash`** so an approval can't be replayed for a different operation; approvals are **single-use** (consumed on first check) and **OTP is lockout-bounded** (≤5 tries). `ApprovalService` BUILT (k256-signed `email-approval-v1`, L1 link / L2 OTP, Resend, approval pages, swept per-key quotas, refuses unpinnable ephemeral key outside dev). **9 runtime integration tests + 7 service tests green.** **Remaining:** dev env config (attestation key pair, Resend key, expose-links), then run the e2e gate | `e2e/tests/api/email-approval.spec.ts` (built): two-phase L2 flow + **operation-binding match/mismatch + replay→consumed**; `tests/it.rs` rejects tampered/expired/misbound/unbound/wrong-operation attestations in-TEE |
...
**Live-key results (2026-06-11, later).** Founder supplied Coinbase CDP + Hyperliquid testnet agent keys → `verify-live.mjs` **11/11 PASS**: Coinbase authenticated reads live (real account); Hyperliquid full testnet lifecycle live with a real **agent** key — deep GTC rest/list/cancel plus an IOC fill round-trip (buy 0.01 ETH → position → reduce-only close → flat), run twice. Two product fixes came out of it: (1) reads now auto-resolve an agent key's MASTER via `userRole` (the agent's own account is always empty — exactly the footgun a Flows integration would have hit); (2) CDP PEMs with literal `\n` (the JSON-download format) parse correctly. Operational note (corrected after founder checked the UI): this account is a Hyperliquid **unified account** — the venue's newer mode where perps margin directly from the unified/spot balance and manual spot↔perp transfers are disabled (which is why fills cleared while the perp-only balance read 0). `fetchBalances` now merges perp equity + spot/unified USDC (preferring the venue's `tokenToAvailableAfterMaintenance` for `free`). On legacy split accounts the transfer is user-signed (`usdClassTransfer`) — the agent cannot move funds, by design.
...
- **Approval store is in-memory (v1)** — pending approvals don't survive a restart/cutover and must complete against the issuing instance; attested approvals are stateless and unaffected. Acceptable for dev/M3 validation. **Not GA-ready on the horizontally-scaled, ping-pong-redeployed fleet** — every cutover lands on a fresh box and drops pending approvals. Fix is designed in **D6.1**: untrusted shared store (Neon over HTTP), integrity kept in the attestation signature, OTP HMAC'd for confidentiality, single-use pushed to the execution-layer idempotency key. Land D6.1 before GA.
