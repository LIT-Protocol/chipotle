...
-- chain_checkpoint existed for the abandoned listener path and is dropped by a
-- follow-up migration once the browser tx-claim flow becomes the only runtime.
```
...
- `chain_checkpoint` — legacy listener resume point dropped by the tx-claim simplification migration.
...
### 🚧 3. Feature 2: LITKEY → Stripe credit — IN PROGRESS
   ...
   **3c.** 🚧 Browser tx-hash claim verification + `litkey_payments` — STARTED IN PR WORKTREE
   ...
   - Migrations: `litkey_payments`, plus a follow-up `DROP TABLE IF EXISTS chain_checkpoint` migration for the removed listener path.
