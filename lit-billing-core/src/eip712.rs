...
//!
//! ## Why this lives in `lit-billing-core`
//!
//! Both `lit-api-server` (account-management endpoints) and `lit-payments`
//! (the `BillingAuth` Rocket guard) need this verifier. Previously the
//! verifier lived only on `lit-api-server`, and `lit-payments` reached it
//! via an HTTP hop (`POST /internal/verify_wallet_auth`). The hop was a
//! pure-function call dressed in network plumbing; folding the verifier
//! into the shared crate lets `lit-payments` invoke it directly.
//!
//! The `chain_id` is supplied per call — the server determines it from its
//! own config — so this module has no global state of its own. That keeps
//! the test surface trivial and removes the lit-api-server `OnceLock`
//! dependency that the original code carried.
...
pub const EIP712_DOMAIN_NAME: &str = "Lit ChainSecured";
...
pub const PRIMARY_TYPE_CREATE_WALLET: &str = "CreateWallet";
pub const PRIMARY_TYPE_CONVERT_ACCOUNT: &str = "ConvertAccount";
pub const PRIMARY_TYPE_ADD_USAGE_API_KEY: &str = "AddUsageApiKey";
pub const PRIMARY_TYPE_BILLING_AUTH: &str = "BillingAuth";

/// Failure modes from [`verify_eip712_signature`]. Two flavours:
/// - [`Eip712Error::BadRequest`] — caller-side problem (bad payload,
///   wrong domain/chain/schema, stale/future timestamp, signature mismatch).
///   Maps to 400/401 on the HTTP edge.
/// - [`Eip712Error::Internal`] — server-side problem computing the digest.
///   Maps to 500.
///
/// Each variant carries the human-friendly summary as a separate field so
/// callers can surface a stable message while logging the wrapped error
/// detail; that's how the original lit-api-server `ApiStatus`-based shape
/// worked and what the dashboard depends on.
#[derive(Debug, thiserror::Error)]
pub enum Eip712Error {
    #[error("{summary}: {detail}")]
    BadRequest { summary: String, detail: String },
    #[error("{summary}: {detail}")]
    Internal { summary: String, detail: String },
}
...
impl Eip712Error {
    ...
    /// Whether this error should be treated as a 4xx (caller fault) vs 5xx
    /// (server fault). Callers may want to translate it into their own
    /// response type.
    pub fn is_bad_request(&self) -> bool {
        matches!(self, Self::BadRequest { .. })
    }
    /// Stable summary string — the second argument the original ApiStatus
    /// helper took. Suitable for surfacing to the dashboard. The detail
    /// field is the wrapped error and is meant for server logs.
    pub fn summary(&self) -> &str {
        match self {
            Self::BadRequest { summary, .. } | Self::Internal { summary, .. } => summary,
        }
    }
}
...
/// against that. The same JSON is also fed to `alloy_dyn_abi::TypedData`
...
/// reads it from its own config — `lit-api-server` from `GLOBAL_NODE_CONFIG`,
/// `lit-payments` from its `Config.chain_id` env var.
///
...
pub fn verify_eip712_signature(
    ...
    let serialized = serde_json::to_string(typed_data_json)
        .map_err(|e| Eip712Error::bad(e.to_string(), "typed_data not serializable"))?;
    if serialized.len() > MAX_TYPED_DATA_LEN {
        return Err(Eip712Error::bad(
            format!("typed_data exceeds {MAX_TYPED_DATA_LEN}-byte cap"),
    ...
    if view.primary_type != expected_primary_type {
        return Err(Eip712Error::bad(
            format!(
                ...
                view.primary_type, expected_primary_type
            ),
    ...
    if !is_known_primary_type(&view.primary_type) {
        ...
        return Err(Eip712Error::bad(
            format!("Unknown primaryType {:?}", view.primary_type),
            "Unknown primaryType",
        ));
    }

    validate_domain(&view.domain, expected_chain_id)?;
    ...
    let digest = typed_data.eip712_signing_hash().map_err(|e| {
        Eip712Error::internal(
            format!("eip712_signing_hash failed: {e}"),
...
fn validate_domain(domain: &DomainView, expected_chain_id: u64) -> Result<(), Eip712Error> {
    ...
    let expected_chain_id = U256::from(expected_chain_id);
    let actual_chain_id = domain
        .chain_id
        .as_ref()
        .map(|v| parse_u256_loose(v, "chainId"))
        .transpose()
        .map_err(|e| Eip712Error::bad(e.to_string(), "Invalid chainId in domain"))?;
...
fn normalize_typed_data_input(
    typed_data_json: &serde_json::Value,
) -> Result<serde_json::Value, Eip712Error> {
    match typed_data_json {
        serde_json::Value::String(s) => serde_json::from_str(s).map_err(|e| {
            Eip712Error::bad(e.to_string(), "typed_data is a string but not valid JSON")
        }),
        other => Ok(other.clone()),
    }
}

fn validate_type_schema(view: &TypedDataSchemaView) -> Result<(), Eip712Error> {
    let expected_payload = payload_field_schema();
    let primary_decl = view.types.get(&view.primary_type).ok_or_else(|| {
        Eip712Error::bad(
            format!("Missing types[{}]", view.primary_type),
            "Missing primaryType in types",
        )
    })?;
    if primary_decl.as_slice() != expected_payload.as_slice() {
        return Err(Eip712Error::bad(
            format!("primaryType field schema mismatch — expected {expected_payload:?}"),
    ...
    let domain_decl = view.types.get("EIP712Domain").ok_or_else(|| {
        Eip712Error::bad(
            "Missing types[EIP712Domain]",
            "Missing EIP712Domain in types",
        )
    })?;
    if domain_decl.as_slice() != expected_domain.as_slice() {
        return Err(Eip712Error::bad(
            format!("EIP712Domain field schema mismatch — expected {expected_domain:?}"),
...
fn extract_address(view: &TypedDataSchemaView) -> Result<Address, Eip712Error> {
    let address_val = view
        .message
        .get("address")
        .ok_or_else(|| Eip712Error::bad("Missing message.address", "Missing message.address"))?;
    let address: Address = serde_json::from_value(address_val.clone())
        .map_err(|e| Eip712Error::bad(e.to_string(), "Invalid message.address"))?;
    Ok(address)
}

fn extract_issued_at(view: &TypedDataSchemaView) -> Result<i64, Eip712Error> {
    let issued_val = view
        .message
        .get("issuedAt")
        .ok_or_else(|| Eip712Error::bad("Missing message.issuedAt", "Missing message.issuedAt"))?;
    ...
    let n = parse_u256_loose(issued_val, "issuedAt")
        .map_err(|e| Eip712Error::bad(e.to_string(), "issuedAt not a uint256 value"))?;
    let i64_max_as_u256 = U256::from(i64::MAX as u64);
    if n > i64_max_as_u256 {
        return Err(Eip712Error::bad(
            format!("issuedAt exceeds i64::MAX ({})", i64::MAX),
...
fn validate_timestamp(issued_at: i64) -> Result<(), Eip712Error> {
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map_err(|e| Eip712Error::internal(e.to_string(), "System clock is before the Unix epoch"))?
        .as_secs() as i64;
    ...
    if delta.abs() > TIMESTAMP_SKEW_SECONDS as i128 {
        return Err(Eip712Error::bad(
            format!(
                "Issued At {issued_at} is outside the ±{TIMESTAMP_SKEW_SECONDS}s window from now ({now})"
            ),
...
mod tests {
    ...
    /// All tests share the Anvil chain id (matches the original
    /// lit-api-server fixture). The verifier takes chain_id as a parameter
    /// now, so there's no `OnceLock` to seed.
    const TEST_CHAIN_ID: u64 = 175188;
    ...
    fn happy_path_all_primary_types() {
        for primary in [
            ...
            let wallet = PrivateKeySigner::random();
            let (typed, sig) = sign_canonical(&wallet, primary, now_secs(), TEST_CHAIN_ID);
            let recovered = verify_eip712_signature(&typed, &sig, primary, TEST_CHAIN_ID)
    ...
    fn rejects_i64_min_issued_at() {
        let typed = serde_json::json!({
            ...
            "domain": {
                ...
                "version": "1",
                "chainId": TEST_CHAIN_ID.to_string(),
            },
            "message": {
    ...
    fn accepts_hex_issued_at() {
        ...
        let recovered =
            verify_eip712_signature(&json, &sig_hex, PRIMARY_TYPE_CREATE_WALLET, TEST_CHAIN_ID)
    ...
    fn rejects_malformed_typed_data_json() {
        ...
        let err =
            verify_eip712_signature(&bogus, "0x00", PRIMARY_TYPE_CREATE_WALLET, TEST_CHAIN_ID)
