...
```sh
npm install @lit-protocol/keychain@2.1.2
npx @lit-protocol/keychain@2.1.2 init ./agent-identity.json
```
...
full catalog compiled into the client. `npx @lit-protocol/keychain@2.1.2 actions` prints it from the CLI and
`npx @lit-protocol/keychain@2.1.2 use <identity> <name> '<json-input>'` runs one.

CLI reads write the requested result to stdout. Avoid sending credential output to logs.
`npx @lit-protocol/keychain@2.1.2 --help` prints usage and `--version` prints the
...
```sh
npx @lit-protocol/keychain@2.1.2 get ./agent-identity.json API_KEY
```
...
```sh
npx @lit-protocol/keychain@2.1.2 run ./agent-identity.json -- stripe balance retrieve
npx @lit-protocol/keychain@2.1.2 run ./id.json --only DATABASE_URL -- psql
npx @lit-protocol/keychain@2.1.2 run ./id.json --env OPENAI_PROD=OPENAI_API_KEY -- python agent.py
```
...
```sh
npx @lit-protocol/keychain@2.1.2 run ./id.json --file GCP_SA=/tmp/sa.json -- \
  env GOOGLE_APPLICATION_CREDENTIALS=/tmp/sa.json gcloud storage ls
npx @lit-protocol/keychain@2.1.2 run ./id.json --file KUBECONFIG_PROD=./kubeconfig -- \
...
```sh
npx @lit-protocol/keychain@2.1.2 attest                 # prints the full report for the default origin
```
...
# Codex CLI
codex mcp add lit-keychain -- npx -y @lit-protocol/keychain@2.1.2 mcp /absolute/path/agent-identity.json
```
...
where the action declares one, `input`; `list_actions` or `npx @lit-protocol/keychain@2.1.2 actions` shows
...
| Artifact       | Shape                                                                                                    |
| -------------- | -------------------------------------------------------------------------------------------------------- |
| Agent identity | JSON from `npx @lit-protocol/keychain@2.1.2 init`: `{ v: 2, privateKey: <64 hex>, publicKey: <64 hex> }` |
