// One-shot setup for the zcash-signer example.
//
...
//   4. Derive the action's Zcash t1 address (runs the action's "address" branch)
//   5. Register the action with the account (metadata)
//   6. Add the specific action CID to the group (audit trail)
//
// There is no contract to deploy: the Zcash t1 address IS the wallet, derived
// from the action's CID. Fund it with a little ZEC (`npm run address` to get
// the address, `npm run balance` to check it), then send (`npm run transfer`).
//
...
async function main() {
  ...
  // -------------------------------------------------------------------------
  console.log("Step 4/6: Deriving the action's Zcash t1 address...");
  ...
  env.upsert("ZCASH_ADDRESS", address);
  console.log(`  ZCASH_ADDRESS=${address}`);
  ...
  console.log("  Zcash address: ", process.env.ZCASH_ADDRESS);
  console.log("  Group ID:      ", process.env.GROUP_ID);
  console.log("\nNext: fund the wallet with a little ZEC, then send.");
  console.log("  npm run address                        # re-derive the address");
  console.log("  npm run balance                        # check its balance");
  console.log("  npm run transfer -- <t1recipient> 0.001  # sign + send 0.001 ZEC");
}
...
async function addGroup(base, apiKey) {
  const body = await call(base, apiKey, "add_group", {
    method: "POST",
    body: JSON.stringify({
      group_name: "zcash-signer",
      group_description: "Action-derived keyless Zcash transparent wallet (secp256k1)",
      pkp_ids_permitted: [],
...
async function addAction(base, apiKey, cid) {
  return call(base, apiKey, "add_action", {
    ...
    body: JSON.stringify({
      action_ipfs_cid: cid,
      name: "zcashSigner",
      description: "Keyless Zcash transparent wallet bound to the action CID; signs capped P2PKH transfers (ZIP-243)",
    }),
  });
}
...
async function createUsageApiKey(base, apiKey, groupId) {
  const body = await call(base, apiKey, "add_usage_api_key", {
    method: "POST",
    body: JSON.stringify({
      name: "zcash-signer-executor",
      description: "Scoped key used by the demo scripts to execute the zcash signer action",
