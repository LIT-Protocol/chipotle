// One-shot setup for the lit-solver-vault example.
//
// What you provide (in .env before running):
//   LIT_API_KEY                Account-level (master) Lit API key
//   ALCHEMY_BASE_SEPOLIA_URL   base-sepolia.g.alchemy.com URL with your key
//   DEPLOYER_PRIVATE_KEY       EOA used to deploy + own the vault (Base-Sepolia gas)
//
// Two cryptographic identities at play:
//
//   * POLICY_SIGNER_ADDRESS — the action's derived wallet address, computed
//     from solverPolicy.js's IPFS CID. The deployed SolverVault pins this as
//     the only signer whose authorization releases a fill. Editing the policy
//     produces a new CID and therefore a new address.
//
//   * LIT_USAGE_API_KEY — a scoped usage key with execute permission inside
//     this example's group. fill.js / the attack scripts use it; the master
//     LIT_API_KEY can't execute /lit_action for actions in your own groups.
//
// Steps, in order:
//   1. Compute the action's IPFS CID
//   2. Create a permission group (wildcard action allowlist, so the inline
//      address-deriver in step 4 can run alongside the policy action)
//   3. Create a scoped usage API key with execute_in_groups: [groupId]
//   4. Derive the action's wallet address (POLICY_SIGNER_ADDRESS)
//   5. Register the action with the account (metadata)
//   6. Add the action CID to the group (audit trail)
//   7. Deploy + wire the contracts (scripts/deploy.js)
//
// Re-running does a fresh setup top-to-bottom and overwrites the derived keys
// in .env; the previously-minted group/usage key/contracts become orphaned.
// Fine for a docs example — we optimize for "explain every endpoint," not cost.

const fs = require("fs");
const path = require("path");
const { execSync } = require("child_process");
const env = require("./_env");

const ACTION_FILE = path.join(__dirname, "..", "action", "solverPolicy.js");
const DEPLOY_NETWORK = process.env.DEPLOY_NETWORK || "baseSepolia";

// Tiny inline Lit Action that returns the wallet address for any CID.
const ADDRESS_DERIVER_CODE = `
  async function main({ ipfsId }) {
    const walletAddress = await Lit.Actions.getLitActionWalletAddress({ ipfsId });
    return { walletAddress };
  }
`;

async function main() {
  env.load();

  const {
    LIT_API_BASE = "https://api.chipotle.litprotocol.com",
    LIT_API_KEY,
  } = process.env;

  for (const k of ["LIT_API_KEY", "DEPLOYER_PRIVATE_KEY", "ALCHEMY_BASE_SEPOLIA_URL"]) {
    if (!process.env[k]) {
      throw new Error(`${k} is required in .env. Copy .env.example to .env and fill it in.`);
    }
  }

  const actionCode = fs.readFileSync(ACTION_FILE, "utf8");

  console.log("Step 1/7: Computing action CID...");
  const actionCid = await getActionCid(LIT_API_BASE, LIT_API_KEY, actionCode);
  env.upsert("ACTION_IPFS_CID", actionCid);
  console.log(`  ACTION_IPFS_CID=${actionCid}`);

  console.log("Step 2/7: Creating group (wildcard action allowlist)...");
  const groupId = await addGroup(LIT_API_BASE, LIT_API_KEY);
  env.upsert("GROUP_ID", String(groupId));
  console.log(`  GROUP_ID=${groupId}`);

  console.log("Step 3/7: Creating scoped usage API key...");
  const usageKey = await createUsageApiKey(LIT_API_BASE, LIT_API_KEY, groupId);
  env.upsert("LIT_USAGE_API_KEY", usageKey);
  console.log(`  LIT_USAGE_API_KEY=${usageKey.slice(0, 12)}... (full key written to .env)`);

  console.log("Step 4/7: Deriving policy signer address from CID...");
  const signerAddr = await deriveActionWalletAddress(LIT_API_BASE, usageKey, actionCid);
  env.upsert("POLICY_SIGNER_ADDRESS", signerAddr);
  console.log(`  POLICY_SIGNER_ADDRESS=${signerAddr}`);

  console.log("Step 5/7: Registering action with account...");
  await addAction(LIT_API_BASE, LIT_API_KEY, actionCid);

  console.log("Step 6/7: Adding action to group...");
  await addActionToGroup(LIT_API_BASE, LIT_API_KEY, groupId, actionCid);

  console.log(`Step 7/7: Deploying + wiring contracts on ${DEPLOY_NETWORK}...`);
  execSync(`npx hardhat run scripts/deploy.js --network ${DEPLOY_NETWORK}`, {
    stdio: "inherit",
    cwd: path.join(__dirname, ".."),
  });
  env.load();

  console.log("\n✓ Setup complete.\n");
  console.log("  Action CID:        ", process.env.ACTION_IPFS_CID);
  console.log("  Policy signer:     ", process.env.POLICY_SIGNER_ADDRESS);
  console.log("  Group ID:          ", process.env.GROUP_ID);
  console.log("  MockUSDC:          ", process.env.MOCK_USDC_ADDRESS);
  console.log("  MockSettlement:    ", process.env.MOCK_SETTLEMENT_ADDRESS);
  console.log("  SolverVault:       ", process.env.SOLVER_VAULT_ADDRESS);
  console.log("  Sample order id:   ", process.env.SAMPLE_DEPOSIT_ID);
  console.log("\nWalk the demo:");
  console.log("  npm run fill                # happy path: legit fill clears policy");
  console.log("  npm run attack:exfiltrate   # redirect to attacker → rejected");
  console.log("  npm run attack:bad-fill     # over the cap → rejected");
  console.log("  npm run policy -- --max 50  # lower the cap live, no key rotation");
  console.log("  npm run policy -- --kill on # engage the kill switch");
  console.log("  npm run exit                # sweep inventory to the cold wallet");
}

// ---------------------------------------------------------------------------
// Lit Chipotle REST helpers.
// ---------------------------------------------------------------------------

async function call(base, apiKey, path, init = {}) {
  const res = await fetch(`${base}/core/v1/${path}`, {
    ...init,
    headers: {
      "X-Api-Key": apiKey,
      "Content-Type": "application/json",
      ...(init.headers || {}),
    },
  });
  const body = await res.json();
  if (!res.ok) {
    const msg = body.message || body.error || JSON.stringify(body);
    const err = new Error(`${path} -> ${res.status}: ${msg}`);
    err.status = res.status;
    err.body = body;
    throw err;
  }
  return body;
}

async function getActionCid(base, apiKey, code) {
  return call(base, apiKey, "get_lit_action_ipfs_id", {
    method: "POST",
    body: JSON.stringify(code),
  });
}

async function deriveActionWalletAddress(base, apiKey, cid) {
  // /lit_action wraps the action's return as { response, logs, has_error }.
  const body = await call(base, apiKey, "lit_action", {
    method: "POST",
    body: JSON.stringify({
      code: ADDRESS_DERIVER_CODE,
      js_params: { ipfsId: cid },
    }),
  });
  if (body.has_error) {
    throw new Error(`address derivation failed: ${body.logs || JSON.stringify(body)}`);
  }
  const result = body.response;
  if (!result || !result.walletAddress) {
    throw new Error(`address derivation returned: ${JSON.stringify(body)}`);
  }
  return result.walletAddress;
}

async function addGroup(base, apiKey) {
  const body = await call(base, apiKey, "add_group", {
    method: "POST",
    body: JSON.stringify({
      group_name: "lit-solver-vault",
      group_description: "Policy-gated fill authorization for a solver vault",
      pkp_ids_permitted: [],
      // U256(0) = "any action allowed in this group." Bounded by the scoped
      // usage key (step 3), which only has execute permission inside THIS group.
      cid_hashes_permitted: ["0"],
    }),
  });
  return body.group_id;
}

async function addAction(base, apiKey, cid) {
  return call(base, apiKey, "add_action", {
    method: "POST",
    body: JSON.stringify({
      action_ipfs_cid: cid,
      name: "solverPolicy",
      description: "Policy gate that authorizes solver fills against the on-chain order",
    }),
  });
}

async function addActionToGroup(base, apiKey, groupId, cid) {
  return call(base, apiKey, "add_action_to_group", {
    method: "POST",
    body: JSON.stringify({ group_id: Number(groupId), action_ipfs_cid: cid }),
  });
}

async function createUsageApiKey(base, apiKey, groupId) {
  const body = await call(base, apiKey, "add_usage_api_key", {
    method: "POST",
    body: JSON.stringify({
      name: "lit-solver-vault-executor",
      description: "Scoped key used by the solver bot to request fill authorizations",
      can_create_groups: false,
      can_delete_groups: false,
      can_create_pkps: false,
      manage_ipfs_ids_in_groups: [],
      add_pkp_to_groups: [],
      remove_pkp_from_groups: [],
      execute_in_groups: [Number(groupId)],
    }),
  });
  if (!body.usage_api_key) {
    throw new Error(`add_usage_api_key returned no key: ${JSON.stringify(body)}`);
  }
  return body.usage_api_key;
}

main().catch((err) => {
  console.error("\nSetup failed:", err.message);
  if (err.body) console.error("Server said:", err.body);
  process.exit(1);
});
