...
| [`mpc-signing`](./mpc-signing) | A threshold-ECDSA key split between a Lit Action and the user (DKLs23 MPC in WASM), 2-of-2 by default (2-of-3 with a cold recovery share). Lit literally cannot sign without the user co-signing each operation, and the full key never exists anywhere. Output is a standard ECDSA signature any EVM contract verifies with `ecrecover`. |
