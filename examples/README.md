...
| [`lit-solver-vault-cowswap`](./lit-solver-vault-cowswap) | CoW Protocol version of the solver vault. The vault is the allowlisted solver, the Lit Action verifies a trader-signed CoW order and builds the whole `settle()` batch, then signs only that settlement. Runs Tier 1 against a self-deployed real GPv2Settlement on Base Sepolia; measured policy authorization latency is **~361 ms median warm** before the solver submits `settle()`. Keyless. |
