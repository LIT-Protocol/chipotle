...
// load(): parse .env into process.env. By default only sets keys not already
// present (so command-line overrides win). Pass force=true to overwrite — used
// after a child deploy process has written fresh values to the file that this
// process loaded stale at startup.
function load(force = false) {
  if (!fs.existsSync(ENV_PATH)) return;
  for (const line of fs.readFileSync(ENV_PATH, "utf8").split("\n")) {
    const m = line.match(/^\s*([A-Z_][A-Z0-9_]*)\s*=\s*(.*?)\s*$/);
    if (!m) continue;
    if (force || !process.env[m[1]]) {
      let val = m[2];
      // Strip an inline comment (whitespace + #...) on unquoted values, so a
      // line like `COW_SELL_AMOUNT=100   # mUSDC` yields "100", not the comment.
