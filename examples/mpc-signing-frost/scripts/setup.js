// One-shot setup for the mpc-signing-frost example.
//
// What you provide (in .env before running):
//   LIT_API_KEY              Account-level (master) Lit API key
//
// Like mpc-signing-ecdsa, setup does NOT create the signer — the Solana
// account is the address produced by the distributed key generation, which
// doesn't exist until you run `npm run keygen`. So the flow is:
//   npm run setup  ->  npm run keygen  ->  npm run fund  ->  npm run sign
//
// What this script does, in order:
//   1. Mint a PKP — the Encrypt/Decrypt boundary that seals the action's MPC
//      session + signing share to THIS action's CID. (It signs nothing itself.)
...
// fall back to source. Must match client/mpcClient.js so CIDs line up.
...
async function main() {
  ...
  // -------------------------------------------------------------------------
  console.log("Step 1/7: Minting PKP (seal boundary for the action's signing share)...");
  ...
  console.log("\nNext:");
  console.log("  npm run keygen                          # interactive 2-of-3 DKG; prints your Solana address");
  console.log("  npm run fund                            # send the address a little SOL (or airdrop on devnet)");
...
async function addAction(base, apiKey, cid) {
  return call(base, apiKey, "add_action", {
    ...
    body: JSON.stringify({
      action_ipfs_cid: cid,
      name: "mpcSigner",
      description: "Lit-side party of a 2-of-3 threshold-FROST (Ed25519) signer",
    }),
  });
}
...
async function createUsageApiKey(base, apiKey, groupId) {
  const body = await call(base, apiKey, "add_usage_api_key", {
    method: "POST",
    body: JSON.stringify({
      name: "mpc-signing-frost-executor",
