...
This is exactly how Flows creates ChainSecured wallets today. Keep using `create_wallet_with_signature` if you want the server-side proof that the caller controls the wallet *before* a PKP is minted (e.g. to keep unauthenticated callers from minting unattached PKPs against your integration).
</Note>
