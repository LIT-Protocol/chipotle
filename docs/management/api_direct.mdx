...
Flips a managed (API-mode) account to ChainSecured (unmanaged) in a single on-chain transaction. The account's `apiKeyHash` is preserved — groups, PKPs, action metadata, usage API keys, and Stripe credit balance all stay attached. Only the admin wallet and the `managed` flag change. There is no reverse path.

Requires the existing master API key in `X-Api-Key`. The server's api\_payer signs the on-chain conversion after verifying the EIP-191 signature; the new admin must sign a SIWE-style message identical in shape to `create_wallet_with_signature`.
