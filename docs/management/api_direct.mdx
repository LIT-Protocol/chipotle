...
Optionally send `X-Privacy-Mode: true` on any request. The server then tags the request ID and suppresses its own log events for that request, so request details (including `js_params` and action output) are not written to Chipotle's server logs. Use it whenever an action call carries anything you would not want in an operator's logs. The [Lit Agent Keychain](https://github.com/LIT-Protocol/chipotle/tree/main/lit-agent-keychain) sets it on every call.
...
- **`POST /get_lit_action_ipfs_id`** — Compute the IPFS CID for a given JS code string. Body is a JSON string. No auth required. This is the same hash the server derives from `code` on every `/lit_action` call, so there is no separate upload or publish step: compute the CID, add it to a group, and send the source. For computing it locally (recommended in production) see [Derived Actions](/lit-actions/derived-actions#3-compute-the-cid-locally).
