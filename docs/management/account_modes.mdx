...
If you need to rely on a permission change right after it lands, don't
sleep a fixed amount — **poll the real path** (retry the action with the
real key until it behaves as expected). The window is normally ~one poll
interval (~10s), but stretches toward the TTL bounds above on a lagging or
just-restarted instance, so a fixed wait is usually too long and
occasionally too short. This is the same "poll, don't sleep" guidance the
[derived-actions guide](/lit-actions/derived-actions) gives for newly
granted permissions.
