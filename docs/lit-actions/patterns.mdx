...
Patterns on this page are single-action techniques. Two larger patterns that combine them have their own pages, and are worth reading first if you are building anything multi-user:

- [**Derived Actions**](/lit-actions/derived-actions) — stamp a config constant into an audited template to mint a new immutable action, with its own TEE-held key, per user, secret, or tenant.
- [**Signed Data in Untrusted Storage**](/lit-actions/signed-storage) — have one action sign the records you store so other actions can verify them at runtime, turning your database into a cache that cannot forge permissions.

On this page:

| Pattern | Use it when |
|---|---|
| [Hostname-Pinned RPC Trust Anchors](#hostname-pinned-rpc-trust-anchors) | The action reads chain state and callers supply the RPC URL |
| [Multi-Source Consensus](#multi-source-consensus) | The action signs data from more than one external source |
| [Gating Logic](#gating-logic--aka-access-control-conditions) | You want access-control conditions, written as plain JavaScript |
| [Action-Identity Signing](#action-identity-signing--immutable-proofs) | The proof must be tied to the code, not to a wallet |
| [A Wallet Bound to the Action](#a-wallet-bound-to-the-action--and-a-unique-one-per-user) | Each user needs a signer only one piece of code can operate |
| [PKP Wallets as Data Vaults](#encrypt--decrypt--pkp-wallets-as-data-vaults) | Several actions share access to encrypted data |
| [Securing RPC URLs](#securing-rpc-urls--hiding-api-keys-with-encryption) | An API key lives inside a URL |
| [Multiple PKPs in a Single Action](#multiple-pkps-in-a-single-action--separating-app-secrets-from-user-signing) | One execution needs an app secret and a per-user signer |
...
1. Retrieve the action's public key or wallet address. `Lit.Actions.getLitActionPublicKey({ ipfsId })` and `getLitActionWalletAddress({ ipfsId })` work from inside any action, for any CID, with no group permission. From outside Lit, run a small fixed helper action that returns the key for a CID you pass in, and cache the result. The helper is shown in [Derived Actions](/lit-actions/derived-actions#discovering-an-actions-public-key-without-running-it).
...
<Warning>
Fetch the public key directly from the Lit origin you trust. Never accept an action's public key from an intermediary such as your own backend, a relayer, or a value embedded in the response you are trying to verify. A compromised intermediary could substitute a key it controls and the signature would still "verify".
</Warning>
...
<Note>
This is the simplest **derived action**: one audited template plus a stamped-in constant. The same move scales to per-secret encryption keys, per-tenant signers, and issuer/consumer pairs that trust each other by CID. [Derived Actions](/lit-actions/derived-actions) covers computing the CID locally, enrolling it, handing out execute-only keys, and keeping template bytes stable so keys do not silently change.
</Note>
...
A full runnable version — deposit an ERC-20 into the wallet, withdraw by signing, and a wrong-user attack the action refuses — is in [`examples/action-bound-wallet/`](https://github.com/LIT-Protocol/chipotle/tree/main/examples/action-bound-wallet) (walkthrough in [Examples §14](./examples#14-a-unique-immutable-wallet-per-user-bound-to-the-action)).

<Warning>
**Use this as a signer, not as the place funds live.** The address is a function of the action bytes, so any change to the template gives every user a new address, and funds sitting at the old one have to be swept by the old code. The runnable example deposits tokens directly at the derived address to stay self-contained; for real balances, make the per-user derived key the owner or signer of a per-user smart-contract wallet, or use a PKP per user and let the action sign with `getPrivateKey({ pkpId })`. See [Which one should hold the funds?](/lit-actions/index#which-one-should-hold-the-funds)
</Warning>
...
A PKP vault is the right shape when **several actions** need the same data, or the decrypt code will evolve. When exactly one immutable action should ever be able to decrypt, skip the PKP and encrypt to the action's own key instead. See [Encrypt to a derived action](/lit-actions/secrets#two-ways-to-hold-a-secret).
</Note>
