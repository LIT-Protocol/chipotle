...
|---|---|---|
| A signer only one user can control | Mint a PKP, add it to a group, gate on the user's signature inside the action | Stamp the user's address into the template. The CID-derived key signs only for them. |
...
A derived key is a great **signer** and a poor **vault**. Its address is a function of the template bytes, so any change (a dependency bump, a bug fix, a new feature) gives every instance a new address. That is harmless when the address holds nothing but authority: a verifier pins a new signer, a smart-contract wallet rotates an owner, a policy names a new key. It is painful when the address holds funds, because every code change becomes a migration that the *old* code must already know how to perform.

So keep balances somewhere that survives a code change and give the derived key the power to move them:

- **Proofs and attestations.** The derived key signs; a contract or API verifies with `ecrecover`. Nothing is held.
- **Smart-contract wallet signer.** Make the derived key an owner on a Safe, a validator on an ERC-4337 account, or the authorized caller of your own contract. The contract holds the funds. When you ship a new template, the contract's existing owners rotate in the new derived address and the balance never moves.
- **Authorizer for a PKP.** The derived action verifies the user and signs an authorization; a PKP-backed action (or the same action using `getPrivateKey({ pkpId })`) executes it. Audited code decides, a durable address pays.

If the Lit key itself must hold the funds, use a PKP. The full comparison is in [Which one should hold the funds?](/lit-actions/index#which-one-should-hold-the-funds)

The [action-bound wallet example](/lit-actions/examples#14-a-unique-immutable-wallet-per-user-bound-to-the-action) deposits tokens directly at the derived address to keep the demo self-contained. Read it as a demonstration of the identity property, not as a recommendation to park real balances there.
...
- **Do not park funds at a derived address.** Hold them in a PKP or a smart-contract wallet and let the derived key sign. See [Signer, not treasury](#signer-not-treasury).
