...
(async () => {
  ...
  if (typeof price !== "number") {
    Lit.Actions.setResponse({ response: JSON.stringify({ error: "Price fetch failed" }) });
    return;
  }
  ...
  Lit.Actions.setResponse({
    response: JSON.stringify({ price, payload, signature }),
  });
})();
```
...
(async () => {
  ...
  if (typeof temp !== "number") {
    Lit.Actions.setResponse({ response: JSON.stringify({ error: "Weather fetch failed" }) });
    return;
  }
  ...
  if (temp < minTempCelsius) {
    Lit.Actions.setResponse({
      response: JSON.stringify({
        signed: false,
        reason: `Temperature ${temp}°C is below threshold of ${minTempCelsius}°C`,
      }),
    });
    return;
  }
  ...
  Lit.Actions.setResponse({
    response: JSON.stringify({ signed: true, temp, message, signature }),
  });
})();
```
...
(async () => {
  ...
  Lit.Actions.setResponse({
    response: JSON.stringify({ holder: holderAddress, balance: balanceFormatted, symbol, payload, signature }),
  });
})();
```
