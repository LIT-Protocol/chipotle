...
// }
const RPC_HOSTS = {
  84532:  /^base-sepolia\.g\.alchemy\.com$/i,  // base sepolia
  421614: /^arb-sepolia\.g\.alchemy\.com$/i,   // arbitrum sepolia
};
...
async function main({
  ...
  // Hostname-whitelist the RPC per chain id. A caller-supplied chainId check
  // alone is theater (the caller can lie consistently); the hostname pins
  // trust to "TLS says this came from Alchemy's actual servers."
  const hostRe = RPC_HOSTS[Number(srcChainId)];
  if (!hostRe) return { authorized: false, reason: `chainId ${srcChainId} not whitelisted` };
  if (!hostRe.test(new URL(srcRpcUrl).hostname)) {
