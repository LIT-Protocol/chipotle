...
Chipotle has no separate secrets store. Secrets are handled with the same primitives as any encrypted data, and you have two shapes to choose from: a **PKP vault** that several actions can be permitted to open, or a **derived action** whose own code-bound key is the only thing that can ever open it. In both cases the plaintext exists only inside the TEE, and security comes from controlling *which immutable code* can reach the key.

## Two ways to hold a secret

| | PKP vault | Encrypt to a derived action |
|---|---|---|
| Key | Symmetric key derived from a PKP you minted | X25519 key derived (via HKDF) from the action's own identity key |
| Who can decrypt | Any action whose CID is in a group with the vault PKP | Exactly one action: the one whose CID the key came from |
| Encrypt with | `Lit.Actions.Encrypt({ pkpId, message })` inside an action | HPKE in the browser or backend, to a public key the action signed for you. Nothing plaintext ever goes to Lit. |
| Rotating the decrypt code | Add the new CID to the group. Same PKP, same ciphertext. | New template means a new CID and a new key. Re-encrypt to the new action. |
| Adding a second consumer | Add its CID to the group | Not possible by design. Have the one action return a policy-gated result instead. |
| Setup | Mint PKP, create group, add PKP and CIDs | Compute CID, add to group |
| Best for | App secrets shared across your actions; data several workflows read | Per-user or per-tenant credentials; anything where "one audited program and nothing else" is the requirement |

The rest of this page covers the PKP vault in detail. For encrypt-to-action, the [Lit Agent Keychain](https://github.com/LIT-Protocol/chipotle/tree/main/lit-agent-keychain) is a complete production implementation: every stored secret is its own derived action, the owner encrypts locally to that action's HPKE key, and an agent gets the plaintext (or a policy-restricted projection of it) only by presenting a signed request the action verifies against a signed, owner-approved policy. The building blocks are documented in [Derived Actions](/lit-actions/derived-actions#deriving-an-encryption-key-from-the-action-key) and [Signed Data in Untrusted Storage](/lit-actions/signed-storage).
...
- [Derived Actions](/lit-actions/derived-actions) — encrypt to an action's own key with no PKP; one immutable action per secret.
- [Signed Data in Untrusted Storage](/lit-actions/signed-storage) — store ciphertext and access policies in your own database without trusting it.
