...
The rest of this page covers the PKP vault in detail. For encrypt-to-action, the [Lit Agent Keychain](/keychain) is a complete production implementation: every stored secret is its own derived action, the owner encrypts locally to that action's HPKE key, and an agent gets the plaintext (or a policy-restricted projection of it) only by presenting a signed request the action verifies against a signed, owner-approved policy. The building blocks are documented in [Derived Actions](/lit-actions/derived-actions#deriving-an-encryption-key-from-the-action-key) and [Signed Data in Untrusted Storage](/lit-actions/signed-storage).
