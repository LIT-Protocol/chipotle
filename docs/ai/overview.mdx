...
description: "Run AI in confidential hardware, with control over code, network access, and software updates."
---

Run your own or a vendor’s Docker container in confidential hardware. Choose which services it can connect to, verify the software that runs, and approve updates on-chain.
...
- **A controlled exit.** Permitted outbound traffic passes through an egress proxy. The container deployment permits exact hostnames over TLS on port 443. Wildcard destinations and arbitrary IP addresses are not allowed.
