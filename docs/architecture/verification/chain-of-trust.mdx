...
| **device_id** | The `device_id` in `/info` is SHA-256 of the platform device identity. On Phala Cloud CVMs, the platform does not expose a device identity, so this field is SHA-256 of the empty string (`e3b0c44...b855`). This is expected and does not affect the security of the attestation. |
