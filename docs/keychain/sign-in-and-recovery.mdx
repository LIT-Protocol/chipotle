...
<Tabs>
  <Tab title="Google">
    Your browser first generates a fresh Ed25519 session key that lives only in memory and expires after about fifty minutes, within the lifetime of the Google ID token. When it expires, the vault stays open and Keychain asks you to approve with Google again before the next change. A hash of that public key and a random value is passed to Google as the OpenID nonce, so the ID token Google returns is bound to this specific session key.
