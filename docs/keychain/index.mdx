...
Cancelling never deletes your secrets or encrypted backups, and it is not revocation: enrolled actions keep working on Free. Only you delete a secret, and **Delete** on its page does exactly that: every agent is denied on its next request, the encrypted value is removed from Keychain, and the slot is freed. If a paid plan expires while you hold more than five secrets, storage mutations stop, but login, revocation and backups remain. Third-party provider charges (OpenAI, Slack plans, Supabase) are separate.
