...
Start with **Secrets → + Add agent** after owner sign-in: paste the existing public key, name the agent, explicitly select secrets, then **Approve selected secrets**. **Ready to use** confirms the successful approvals. No secret is selected automatically. No config transfer or restart is needed for live clients.

For legacy clients only, **Advanced: legacy static config → Download agent config** remains available. A per-secret **Agent config** names one secret. **Download agent config**, next to an agent under **Authorized agents**, names every secret in the vault that agent is approved for, which is what `keychain run --only A,B` and a single `get`/`use` config expect. The MCP server additionally merges several config files from the same vault.
